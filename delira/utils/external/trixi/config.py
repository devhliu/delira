...
from .json_utils import ModuleMultiTypeDecoder, ModuleMultiTypeEncoder
    ...
    def __init__(self, file_=None, config=None, update_from_argv=False, deep=False, **kwargs):
        ...
        if config is not None:
            if deep:
                ...
                # and get deepcopy
                config = Config(
                    config=config, update_from_argv=False, deep=False)
        ...
        if len(kwargs) >= 1:
            if deep:
                kwargs = Config(
                    config=kwargs, update_from_argv=False, deep=False)
    ...
    def __getitem__(self, key):
        ...
        if key == "":
            if len(self.keys()) == 1:
                ...
                raise KeyError(
                    "Empty string only works for single element Configs.")
        ...
        try:
            if hasattr(file_, "read"):
                ...
                with open(file_, "r") as file_object:
                    new_dict = json.load(
                        file_object, cls=decoder_cls_, **kwargs)
        ...
        for key in all_keys:
            ...
            if not all_equal:
                ...
                if not all_configs:
                    ...
                    difference[key] = Config.difference_config_static(
                        *current_values, only_set=only_set)
            ...
            def items():
                for key, val in obj.items():
                    if isinstance(val, dict) and (isinstance(key, str) or (isinstance(key, int) and flatten_int)):
                        ...
                        keep_this = keep_lists or not isinstance(key, (str, int)) or (
                            isinstance(key, int) and not flatten_int)
...
def update_from_sys_argv(config, warn=False):
    ...
    if len(sys.argv) > 1:
            ...
            if val is None:
                ...
                if type(val) == bool:
                    ...
                    if len(val) > 0 and type(val[0]) != type:
                        parser.add_argument(
                            name, nargs='+', type=type(val[0]), default=val)
                    else:
        ...
        if len(unknown) > 0 and warn:
            warnings.warn("Called with unknown arguments: {}".format(
                unknown), RuntimeWarning)

        # calc diff between configs
        diff_keys = list(Config.difference_config_static(
            param, config_flat).flat().keys())
